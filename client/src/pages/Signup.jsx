...
export default function Signup() {
  ...
  async function handleSignup(e) {
    ...
    try {
      ...
      alert("Registration complete! Check your email or try logging in.");
      navigate("/login");
    } catch (err) {
      alert(err.message);
    } finally {
    ...
    >
      ...
      <div className="bg-white/40 backdrop-blur-2xl border border-white/60 w-full max-w-md p-8 md:p-10 rounded-[2.5rem] shadow-2xl space-y-6 relative z-10">
        <div className="text-center space-y-2">
          ...
          <h2 className="text-3xl font-black text-purple-950">Create Account</h2>
          <p className="text-xs font-bold text-slate-700">Get inspiring design ideas & remodel like a pro!</p>
        </div>

        <form onSubmit={handleSignup} className="space-y-4">
          <div className="space-y-1">
            <label className="text-xs font-extrabold text-purple-950 tracking-wider uppercase pl-1">Full Name</label>
            <input
              type="text"
          ...
          <div className="space-y-1">
            <label className="text-xs font-extrabold text-purple-950 tracking-wider uppercase pl-1">Password</label>
            <input
              type="password"
              placeholder="••••••••"
              value={password}
        ...
        <p className="text-center text-xs font-bold text-slate-800 pt-2">
          ...
          <Link to="/login" className="text-purple-800 font-black hover:underline pl-1">
            Log In
          </Link>
        </p>
      </div>
